...
// CORRIGIDO: shared: false para dados não sumIrem ao atualizar
const DB = {
...
function ReportModal({counting,items,onClose}) {
  ...
  useEffect(()=>{
    ...
    // Margem lateral: 32px dos dois lados (simetrica)
    const MARGIN=32;
    const W=900;
    const rowH=40;
    const hH=110;
    ...
    const H=hH+thH+list.length*rowH+130+50;
    ...
    ctx.fillText("RELATÓRIO DE CONTAGEM",MARGIN,40);
    ctx.font="14px Arial"; ctx.fillStyle="#93c5fd";
    ctx.fillText(counting.label,MARGIN,64);
    ctx.font="12px Arial"; ctx.fillStyle="#93c5fd";
    ctx.fillText(`Data: ${fmtDate(counting.date)}   |   Gerado: ${new Date().toLocaleString("pt-BR")}`,MARGIN,88);

    // Definição de colunas
    // Margem esquerda = MARGIN = 32. Margem direita = MARGIN = 32 (ultima coluna termina em W-MARGIN=868)
    // Total util = W - 2*MARGIN = 836px
    // Proporcoes: Insumo(210) | Unidade(72) | Qtd Contada(82) | Minimo(58) | Maximo(58) | Status(150) | Compra Necessaria(206)
    // Soma = 836 OK
    const cols=[
      {l:"Insumo",            x:MARGIN,      w:210},
      {l:"Unidade",           x:MARGIN+210,  w:72},
      {l:"Qtd Contada",       x:MARGIN+282,  w:82},
      {l:"Mínimo",            x:MARGIN+364,  w:58},
      {l:"Máximo",            x:MARGIN+422,  w:58},
      {l:"Status",            x:MARGIN+480,  w:150},
      {l:"Compra Necessária", x:MARGIN+630,  w:206},
    ];
    // Verificar: ultima coluna termina em 32+630+206 = 868 = W-32 ✓

    let y=hH;
    // Cabecalho das colunas
    ctx.fillStyle="#0a1e3c"; ctx.fillRect(0,y,W,thH);
    ctx.font="bold 11px Arial"; ctx.fillStyle="#bfdbfe";
    cols.forEach(c=>ctx.fillText(c.l,c.x,y+25));
    y+=thH;
    ...
    list.forEach((ci,idx)=>{
      ...
      const cur=ci.counted??0;
      const need=(it.min&&cur<=it.min)?(it.max?Math.max(it.max-cur,0):Math.max(it.min-cur+it.min,0)):0;

      ctx.fillStyle=idx%2===0?"#fff":"#f0f5ff";
      ctx.fillRect(0,y,W,rowH);
      ctx.strokeStyle="#dbe8f8"; ctx.lineWidth=1;
      ctx.beginPath(); ctx.moveTo(0,y+rowH); ctx.lineTo(W,y+rowH); ctx.stroke();

      // Insumo com truncagem
      const name=String(ci.name||it.name||"");
      ctx.font="600 12px Arial"; ctx.fillStyle="#1e293b";
      const maxWName=cols[0].w-6;
      let trName=name;
      while(ctx.measureText(trName).width>maxWName&&trName.length>3) trName=trName.slice(0,-1);
      if(trName!==name) trName=trName.slice(0,-1)+"...";
      ctx.fillText(trName,cols[0].x,y+26);

      // Unidade
      ctx.font="12px Arial"; ctx.fillStyle="#475569";
      const unit=String(it.unit||"—");
      let trUnit=unit;
      while(ctx.measureText(trUnit).width>cols[1].w-4&&trUnit.length>2) trUnit=trUnit.slice(0,-1);
      ctx.fillText(trUnit,cols[1].x,y+26);

      // Qtd contada
      ctx.font="bold 13px Arial"; ctx.fillStyle="#1d4ed8";
      ctx.fillText(String(cur),cols[2].x,y+26);

      // Minimo e Maximo
      ctx.font="12px Arial"; ctx.fillStyle="#475569";
      ctx.fillText(String(it.min||"—"),cols[3].x,y+26);
      ctx.fillText(String(it.max||"—"),cols[4].x,y+26);
      ...
      if(st){
        ...
        ctx.fillStyle=bg;
        ctx.beginPath();
        if(ctx.roundRect) ctx.roundRect(cols[5].x,y+9,bw,20,4);
        else ctx.rect(cols[5].x,y+9,bw,20);
        ctx.fill();
        ctx.font="bold 10px Arial"; ctx.fillStyle=tc;
        let stLbl=st.label;
        while(ctx.measureText(stLbl).width>bw-8&&stLbl.length>2) stLbl=stLbl.slice(0,-1);
        ctx.fillText(stLbl,cols[5].x+6,y+23);
      }

      // Compra necessaria — alinhada a esquerda da coluna, cabe ate W-MARGIN
      if(need>0){
        ctx.font="bold 12px Arial"; ctx.fillStyle="#d97706";
        ctx.fillText(`+${need} ${it.unit||""}`,cols[6].x,y+26);
      } else {
        ctx.font="12px Arial"; ctx.fillStyle="#94a3b8";
        ctx.fillText("—",cols[6].x,y+26);
      }
      y+=rowH;
    });
    ...
    ctx.beginPath();
    if(ctx.roundRect) ctx.roundRect(MARGIN,y,boxW,86,10);
    else ctx.rect(MARGIN,y,boxW,86);
    ctx.fill();
    ...
    ctx.font="bold 13px Arial"; ctx.fillStyle="#0a1e3c";
    ctx.fillText("RESUMO",MARGIN+18,y+24);
    ...
    ctx.fillText(`Total de insumos: ${list.length}`,MARGIN+18,y+46);
    ctx.fillText(`Abaixo do mínimo: ${ab}`,MARGIN+18,y+66);
  ...
  // CORRIGIDO: download via toBlob + createObjectURL (funciona em todos os navegadores)
  const download=()=>{
    const canvas=canvasRef.current; if(!canvas)return;
    const nome=counting.label.replace(/\s+/g,"_").replace(/[^\w-]/g,"");
    canvas.toBlob(blob=>{
      if(!blob)return;
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",zIndex:9999,padding:12,overflowY:"auto",fontFamily:T.fontBase}}>
      <div style={{width:"100%",maxWidth:940,background:T.card,borderRadius:14,border:`1px solid ${T.border}`,overflow:"hidden"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",borderBottom:`1px solid ${T.border}`}}>
          <div style={{fontWeight:700,fontSize:T.fs13,color:T.text}}>Relatório: {counting.label}</div>
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={onClose} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:8,padding:"7px 12px",color:T.textSub,fontWeight:600,fontSize:T.fs12,cursor:"pointer"}}>X Fechar</button>
          </div>
        </div>
...
function sendWA(phone, counting, items) {
  const lines = (counting.items || []).map(ci => {
    ...
    return `  • *${ci.name || it.name}*: ${ci.counted ?? 0}${unit}`;
  }).join("\n");

  const msg =
    `😀 Olá, Teresa!\n\n` +
    `Segue o relatório da *${counting.label}* referente a ${fmtDate(counting.date)}.\n\n` +
    `📋 *Quantidades contabilizadas:*\n${lines}\n\n` +
    `_Sistema de Gestão de Contagens_`;

  const url = `https://wa.me/${normPhone(phone)}?text=${encodeURIComponent(msg)}`;
...
function useAppData() {
  ...
  const reload = useCallback(async()=>{
    ...
    setState({
      ...
      appPass:appPass||DEFAULT_PASS,
      passHint:passHint||null,
...
// CORRIGIDO: sem mostrar mínimo/máximo/compra na área do contador
function CounterView({items,countings,scheduledDates,onSubmit,onBack,whatsapp}) {
  ...
  const doSend=()=>{
    ...
    setSaved(counting);
    setPhase("done");
    // CORRIGIDO: envio via WhatsApp direto
    if(whatsapp) sendWA(whatsapp,counting,items);
  };
      ...
      <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:T.fontBase,display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"18px 16px 0",overflowY:"auto"}}>
          <div style={{width:"100%",maxWidth:400}}>
            {/* CORRIGIDO: card do insumo sem mínimo/máximo */}
            ...
            {isConf?(
              <div style={{...S.card({marginBottom:14,padding:"20px",background:T.green+"0a",border:`1.5px solid ${T.green}30`})}}>
                ...
                {/* Sem status badge e sem barra de estoque no contador */}
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:0,fontFamily:T.fontBase,padding:24}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,width:"100%",maxWidth:340}}>
        {saved&&<button onClick={()=>setShowReport(true)} style={S.btn(T.accent,true)}>📄 Ver Relatório</button>}
          ...
          <div key={it.id} style={{...S.card({marginBottom:10,border:`1px solid ${st?(st.level==="danger"?T.red:st.level==="over"?T.purple:T.green)+"30":T.border}`})}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div style={{flex:1}}>
                ...
                {counted!==undefined&&<div style={{fontSize:T.fs12,marginBottom:8}}>Última contagem: <b style={{color:st?.color||T.text,fontFamily:T.fontMono}}>{counted}</b></div>}
